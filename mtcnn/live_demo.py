...
for frame in camera.capture_continuous(raw_image, format="bgr", use_video_port=True):
	...
	image = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0,0,0])
	...
	for (i, rect) in enumerate(rects):
	    img_copy = cv2.rectangle(img_copy,(rect[0], rect[1]), (rect[0] + rect[2], rect[1]+rect[3]), 255)
